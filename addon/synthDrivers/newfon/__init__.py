...
def setLngModule(language):
	global lngModule
	if language in globals(): lngModule = globals()[language]
	else: 		lngModule = ru
...
class AudioCallback(object):
	...
	def __call__(self, udata, buffer, length):
		if self.__is_silence.is_set(): 				return 1
		self.__player.feed(string_at(buffer, length))
		return 0

class IndexCallback(object):
...
class DoneSpeakingCallback(object):
...
class SynthDriver(SynthDriver):
	...
	def _get_availableInterpolations(self):
		...
		interpolations["2"] = StringParameterInfo("2", _(U"2 X"))
		interpolations["4"] = StringParameterInfo("4", _(U"4 X"))
